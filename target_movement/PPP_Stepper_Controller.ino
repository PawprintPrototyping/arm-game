...
void setup() {
  ...
  if (checkALimitTop() || checkALimitBottom()) {
    fail("Target A is at a limit. Move target away from limits before startup cal.");
  }
  if (checkBLimitTop() || checkBLimitBottom()) {
    fail("Target B is at a limit. Move target away from limits before startup cal.");
  }
